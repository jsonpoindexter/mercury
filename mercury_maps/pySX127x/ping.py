...
class Ping(LoRa):
	...
	if payload == payload_pong: ## if message is PONG [P,o,n,g]/[80, 111, 110, 103] send PING [P,i,n,g]/[80, 105, 110, 103]
		...
		self.set_payload_length(len(payload_ping)) # PING
	        base_addr = self.get_fifo_tx_base_addr()
	        self.set_fifo_addr_ptr(base_addr)
	
		self.spi.xfer([REG.LORA.FIFO | 0x80] + payload_ping)[1:]
        	self.set_mode(MODE.TX)  # send PING
	...
	self.set_payload_length(len(payload_ping))
        base_addr = self.get_fifo_tx_base_addr()
        self.set_fifo_addr_ptr(base_addr)
